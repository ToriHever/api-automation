...
`GetDynamics` поддерживает операторы (`"фраза"`, `!слово`) **только при
`PERIOD_DAILY`**. При `PERIOD_MONTHLY` (то, что использует `dynamics` в этом
сервисе) — только оператор `+`; кавычки/`!` гарантированно возвращают `400
Invalid query`. Дважды пробовали засунуть их в `dynamics` — оба раза API
отбивал реальным запросом. Если когда-нибудь снова понадобятся точные/узкие
варианты частотности — делать отдельный скрипт по образцу
[`wordstat-dynamics-range-daily.js`](../../scripts/wordstat-dynamics-range-daily.js)
(`PERIOD_DAILY`, окно 60 дней), а не встраивать в помесячный `dynamics`.
Откат зафиксирован в `services/common/data/2026-09_rollback_wordstat_match_type.sql`.
...
# Делит ту же часовую квоту 100/ч — не пересекать по дням с dynamics выше
# (сейчас 3-9, запас есть). 290 фраз ≈ 4 часа, поэтому хватает одного дня.
