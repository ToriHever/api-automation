...
-- Порядок приоритета (после правки, см. пункт 6 в шапке файла):
--   1. Сброс неверно проставленного topic_id='ddos атака' с прошлого прогона
--      там, где также есть намерение защититься (корень 'защит').
--   2. Конкретная цель/тип атаки по словарю (сервер/сайт/mitm/sql/...).
--   3. Общее намерение 'защита' (корень 'защит', цель по словарю не нашлась).
--   4. Упоминание ddos в любом написании -> 'ddos атака'.
--   5. Просто слово 'атака' без остального -> удалить / другой вид атаки.
-- ============================================================

-- Дополняем common.topics всеми каноническими значениями из словаря уточнения
-- (часть уже есть из прошлого импорта данных, ON CONFLICT просто пропустит их).
...
-- Шаг 0 (коррекция после первого прогона): сбрасываем topic_id='ddos атака' там,
-- где запрос также выражает намерение защититься — чтобы их пересчитал новый
-- порядок приоритета ниже. Идемпотентно: если такого не осталось, ничего не делает.
...
-- Приоритет 1: словарь конкретных объектов/типов атак — первое совпадение по
-- порядку словаря побеждает (WHERE topic_id IS NULL гарантирует это между шагами).
-- Срабатывает, только если есть корень 'защит' или слово 'атака'.
...
-- Приоритет 2: конкретной цели по словарю не нашли, но есть намерение защититься
-- (корень 'защит') -> общий интент 'защита'. Именно этот шаг чинит кейс
-- 'ddos атака как защититься' — раньше такой запрос перехватывался ddos-упоминанием.
...
-- Приоритет 3: упоминание ddos в любом написании, если ни цель, ни намерение
-- защититься не определились -> 'ddos атака'.
UPDATE common.requests SET topic_id = (SELECT topic_id FROM common.topics WHERE topic_name = 'ddos атака')
WHERE topic_id IS NULL AND request ~* '(\mddos\w*|\mдудос\w*|\mддос\w*|\mdos\w*|\mдосс\w*|\mдоос\w*|\mдос\w*|\md o s\w*|\mдедос\w*|\mдидос\w*|\mдодос\w*|\mдудокс\w*|\mdoss\w*|\mдудоса\w*|\mд дос\w*|\mддс\w*|\mмдос\w*|\mдтос\w*|\mdds\w*|\mdudos\w*|\mдэдос\w*|\mdoc\w*|\mdo dos\w*|\mдосить\w*|\mддосить\w*|\mдоус\w*|\mотказ в обслуживании\w*)';

-- Приоритет 4: слово 'атака' есть, но по словарю ничего конкретного не нашли и
-- намерения защититься/ddos-упоминания тоже нет. Если это общий вопрос-определение
-- (что/это/как/какой/кто такой/кто такие) — помечаем на удаление.
...
-- Иначе — просто неопознанный вид атаки.
