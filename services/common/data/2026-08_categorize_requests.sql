...
-- Новое: 'Цены' — у этого кластера, как и у 'Цели', вообще не было правила
-- (баг переноса из VBA). Любой коммерческий интент (цена/стоимость/сколько
-- стоит/тарифы/купить/заказать) -> 'Цены', НЕЗАВИСИМО от того, выглядит
-- запрос вредоносно или нет — проверяется РАНЬШЕ 'Вредоносные (Покупка/Заказ)',
-- поэтому пересекающиеся слова (купить/заказать/сколько стоит) теперь всегда
-- уходят сюда. Сбрасываем то, что уже могло уйти в 'Вредоносные (Покупка/Заказ)'
-- с прошлых прогонов, чтобы такие строки пересчитались.
...
-- заказ атаки (актуально для того немногого, что 'Цены' выше не заберёт).
UPDATE common.requests SET cluster_id = NULL
WHERE cluster_id = (SELECT cluster_id FROM common.clusters WHERE cluster_name = 'Вредоносные (Покупка/Заказ)')
...
-- Приоритет 6: упоминание ddos в любом написании, если ничего конкретнее не
...
-- Приоритет 7: слово 'атака' есть, но ничего конкретнее не нашли.
