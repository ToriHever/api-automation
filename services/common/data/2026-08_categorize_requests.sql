...
-- 'Цены'/'Вредоносные (Покупка/Заказ)' с прошлых прогонов, чтобы пересчиталось
-- по актуальному приоритету.
...
-- 'атака') + коммерческое слово (купить/заказать/сколько стоит/цена/тариф/
-- услуги/сервис) + НЕТ намерения защититься (корень 'защит') -> 'Вредоносные
-- (Покупка/Заказ)'. Это заказ атаки, а не покупка защиты — должно перехватывать
-- 'Цены' именно в этом узком случае. Если запроса нет ddos ИЛИ есть 'защит' —
-- сюда не попадёт, уйдёт в 'Цены' ниже как обычно.
...
WHERE cluster_id IS NULL
  ...
  AND request ~* '(\mцена\M|\mцены\M|\mценам\M|\mценой\M|\mценах\M|\mцене\M|\mстоимост\w*|\mсколько стоит\w*|\mтариф\w*|\mкупить\w*|\mзаказ\w*|\mуслуг\w*|\mсервис\w*)'
