...
SELECT
    ...
    c.target_url_topvisor AS target_url,
    dpe.project_name,
    dpe.search_engine,
    p.request,
    cr.request_id,
...
COMMENT ON VIEW analytics.topvisor_relevance_visibility IS 'Сырые данные (день × запрос) для мультииндикатора релевантности/видимости группы в DataLens. Частота (wd.frequency) — из wordstat.tmp_dynamics за тот же месяц, коэффициент видимости считается по позиции того дня.';
