...
`Имя алерта` идёт в `alert_name` (свободный текст, не справочник) и всегда
видно префиксом в заголовке заметки в `common.notes`: `Имя алерта — Заголовок статьи`
(разделитель — тире, не квадратные скобки: те в DataLens зарезервированы под
ссылки на поля в формулах, `[ИмяПоля]`).
